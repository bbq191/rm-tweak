//! `pdf-extract` 0.12.1（MIT，Jeff Muizelaar）本地 fork，2026-09-23 引入。字体/glyph 解码（真正难、
//! 易出 CJK/嵌入字体边界问题的部分）原样保留、不重写；只在 `Processor::process_stream` 里改了三处：
//! ① `g`/`G`/`rg`/`RG`/`k`/`K` 六个颜色算子上游完全不处理（只 `dlog!`，不更新 `fill_color`），
//!   现在正确写回 `fill_color`/`fill_colorspace`——不补这个，`OutputDev` 拿到的颜色本来就是错的；
//! ② `output_character` 新增当前填充色参数（RGB，从 `fill_colorspace`+`fill_color` 换算，Gray/RGB/CMYK
//!   都处理，拿不准的 colorspace 保守给黑色而不是 panic）；
//! ③ `Do` 算子原样对**所有** XObject（含 `/Subtype /Image`）当 Form 递归 `process_stream`——图片字节
//!   被当 content stream 解码，逻辑性错误（此前只是静默失败没崩，因为 lopdf 的 content 解码对垃圾字节
//!   够宽容）。现在按 `/Subtype` 区分：Image 不递归，改调用新增的 `OutputDev::output_image` 钩子
//!   （给 CTM + XObject 资源名，不解码像素——像素解码复用调用方已有的 lopdf 0.45 路径）；Form 保持
//!   原递归行为不变。
        ...
        if let Some(w) = w {
            while i < w.len() {
                if let &Object::Array(ref wa) = w[i+1] {
                    ...
                    // `c_first c_last w` 区间写法（PDF 32000-1 §9.7.4.3，闭区间）。上游三处全错：
                    // c_last/c_width 都取成了 `w[i]`、区间半开——`875 28574 1000` 被当成空区间
                    // `875..875`，整段 CID 拿不到宽度、落到 `DW`（calibre 导出的微软雅黑子集 `DW` 显式
                    // 写 0），中文字形宽度全为 0、同一个 Tj 里的字全叠在同一个 x 上（2026-09-23 真机
                    // 《T.E.双语》坐实，cang-jie fork 修正）。
...
fn show_text(gs: &mut GraphicsState, s: &[u8],
    ...
    // 字段级借用（不是借整个 gs），跟下面循环里读 gs.ctm 一样不冲突——颜色本来就该在整个 Tj/TJ 串里
    // 保持不变（改颜色是独立算子，不会夹在一次 show_text 调用中间），算一次够了。
    let fill_rgb = resolve_fill_rgb(&gs.fill_colorspace, &gs.fill_color);
    let ts = &mut gs.ts;
    ...
    for (c, length) in font.char_codes(s) {
        ...
        output.output_character(&trm, w0, spacing, ts.font_size, &font.decode_char(c), fill_rgb)?;
        let tj = 0.;
        let ty = 0.;
...
/// 把当前填充色换算成 RGB 三元组给 `OutputDev` 用。**拿不准就返回 `None`**，不瞎猜一个可能错得
/// 离谱的颜色——比如 Separation/DeviceN 的 tint 值语义跟 DeviceGray 正好相反（tint 1=满墨=最深，
/// gray 1=白），当 gray 直接用会反色；Lab 需要走 CIE 转换公式，没有真实样本核对正确性之前不编。
/// `ICCBased`/`CalRGB`/`CalGray` 按分量数当对应 Device 空间处理——真实 PDF 里绝大多数 ICC 曲线
/// 包的就是这三种之一，跳过 profile 本身的解析，色差可接受。`color` 为空（该算子还没被发过、
/// 沿用初始状态）也返回 `None`。
pub fn resolve_fill_rgb(colorspace: &ColorSpace, color: &[f64]) -> Option<[u8; 3]> {
    let clamp = |v: f64| -> u8 { (v.clamp(0.0, 1.0) * 255.0).round() as u8 };
    match colorspace {
        ColorSpace::DeviceGray | ColorSpace::CalGray(_) if color.len() == 1 => {
            let g = clamp(color[0]);
            Some([g, g, g])
        }
        ColorSpace::DeviceRGB | ColorSpace::CalRGB(_) if color.len() == 3 => {
            Some([clamp(color[0]), clamp(color[1]), clamp(color[2])])
        }
        ColorSpace::DeviceCMYK if color.len() == 4 => {
            let (c, m, y, k) = (color[0], color[1], color[2], color[3]);
            Some([clamp((1.0 - c) * (1.0 - k)), clamp((1.0 - m) * (1.0 - k)), clamp((1.0 - y) * (1.0 - k))])
        }
        ColorSpace::ICCBased(_) => match color.len() {
            1 => resolve_fill_rgb(&ColorSpace::DeviceGray, color),
            3 => resolve_fill_rgb(&ColorSpace::DeviceRGB, color),
            4 => resolve_fill_rgb(&ColorSpace::DeviceCMYK, color),
            _ => None,
        },
        _ => None,
    }
    ...
    fn process_stream(&mut self, doc: &'a Document, content: Vec<u8>, resources: &'a Dictionary, media_box: &MediaBox, output: &mut dyn OutputDev, page_num: u32) -> Result<(), OutputError> {
        ...
        for operation in &content.operations {
            ...
            match operation.operator.as_ref() {
                ...
                // 上游原样只 dlog 不处理这六个——但它们是最常见的直接颜色算子（比 sc/scn 常见得多，
                // 大多数 PDF 生成器写纯黑/纯色文字直接用 g/rg/k，不走带 colorspace 资源字典的 scn），
                // 不补上 fill_color 从头到尾都是空的，颜色特性无从谈起（2026-09-23 补，cang-jie fork）。
                ...
                "Do" => {
                    ...
                    if is_image {
                        // 图片 XObject 没有 content stream 可解——上游原样对所有 Do 一律当 Form 递归，
                        // 图片字节被当算子流解析，逻辑性错误（此前只是静默失败没崩，lopdf 的 content
                        // 解码对垃圾字节够宽容）。只报告 CTM（单位正方形→页面实际矩形）+ 资源名，像素
                        // 解码交给调用方已有的路径（2026-09-23 cang-jie fork 修正）。
                        output.output_image(&gs.ctm, name)?;
                    } else {
...
pub trait OutputDev {
    ...
    /// `color`：当前文字填充色，`None` 表示没能可靠解析（见 [`resolve_fill_rgb`]）——调用方拿到
    /// `None` 应该当作"不确定"处理，不要瞎填一个默认色（cang-jie fork 新增参数，2026-09-23）。
    fn output_character(&mut self, trm: &Transform, width: f64, spacing: f64, font_size: f64, char: &str, color: Option<[u8; 3]>) -> Result<(), OutputError>;
    ...
    /// `Do` 算子命中 `/Subtype /Image` 的 XObject 时调用（cang-jie fork 新增，2026-09-23）——只给
    /// CTM（单位正方形 (0,0)-(1,1) 变换到图片在页面上的实际矩形）和 XObject 资源名，不解码像素：
    /// 像素解码调用方已经有独立的 lopdf 路径（不想在这个 fork 里再长出第二套图片解码逻辑）。
    /// 默认空实现——不需要图片位置的调用方（现有 `HTMLOutput`/`SVGOutput`/`PlainTextOutput`）不用改。
    fn output_image(&mut self, _ctm: &Transform, _xobject_name: &[u8]) -> Result<(), OutputError> { Ok(()) }
}
...
impl<'a> OutputDev for HTMLOutput<'a> {
    ...
    fn output_character(&mut self, trm: &Transform, width: f64, spacing: f64, font_size: f64, char: &str, _color: Option<[u8; 3]>) -> Result<(), OutputError>{
...
impl<'a> OutputDev for SVGOutput<'a> {
    ...
    fn output_character(&mut self, _trm: &Transform, _width: f64, _spacing: f64, _font_size: f64, _char: &str, _color: Option<[u8; 3]>) -> Result<(), OutputError>{
        Ok(())
    }
    fn begin_word(&mut self) -> Result<(), OutputError> {Ok(())}
    fn end_word(&mut self) -> Result<(), OutputError> {Ok(())}
...
impl<W: ConvertToFmt> OutputDev for PlainTextOutput<W> {
    ...
    fn output_character(&mut self, trm: &Transform, width: f64, _spacing: f64, font_size: f64, char: &str, _color: Option<[u8; 3]>) -> Result<(), OutputError> {
        let position = trm.post_transform(&self.flip_ctm);
        let transformed_font_size_vec = trm.transform_vector(vec2(font_size, font_size));
        // get the length of one sized of the square with the same area with a rectangle of size (x, y)
        let transformed_font_size = (transformed_font_size_vec.x*transformed_font_size_vec.y).sqrt();
